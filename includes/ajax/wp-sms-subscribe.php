<?php
	add_action( 'wp_ajax_wp_sms_subscribe', 'wp_sms_subscribe_callback' );
	add_action( 'wp_ajax_nopriv_wp_sms_subscribe', 'wp_sms_subscribe_callback' );

	function wp_sms_subscribe_callback() {
		$data = $_REQUEST['data'];
		$name	= trim($data['name']);
